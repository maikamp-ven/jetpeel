  ...
  return (
    <main className="productPage__section section">
      <Helmet>
        ...
        <meta name="description" content={tDevices(`products.${slug}.text`).substring(0, 160)} />
        <meta name="keywords" content="JetPeel, skincare, beauty, LED therapy" />
        <meta property="og:title" content={tDevices(`products.${slug}.title`)} />
        <meta property="og:description" content={tDevices(`products.${slug}.text`).substring(0, 160)} />
        <meta property="og:image" content={productWithImages.img} />
        <meta property="og:url" content={`https://jetpeel.fi/${slug}`} />
        <script type="application/ld+json">{JSON.stringify(schemaData)}</script>
      </Helmet>

      <img src={productWithImages.headerImg} alt={tDevices(`products.${slug}.title`)} className="product__img-header" />

      <div className="container">
        <h2 className="page__title title-1">{tDevices(`products.${slug}.title`)}</h2>

        <div className="product__hero">
          <div className="product__info">
            <h2 className="product__title title-2">{tDevices(`products.${slug}.heroTitle`)}</h2>
            <p className='product__text'>{tDevices(`products.${slug}.heroText`)}</p>
          </div>
          <div className="product__img-wrapper">
            <img src={productWithImages.img} alt={tDevices(`products.${slug}.title`)} className="product__img" />
          </div>
        </div>

        {productWithImages.sections?.map((section, index) => (
          <div key={index} className={section.imgSection ? "product__section product__section--with-image" : "product__section"}>
            <h2 className="title-2">{tDevices(`products.${slug}.sections.${index}.title`)}</h2>
            ...
            {section.imgSection ? (
              <div className="product__section-content">
                ...
                <div className="section__img-wrapper">
                  <img src={section.imgSection} alt={tDevices(`products.${slug}.sections.${index}.title`)} className="section-image" />
                </div>
              </div>
            ) : (
              section.list && (
